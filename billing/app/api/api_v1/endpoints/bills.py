...
def get_bills(
    ...
    Get bill details
    """
    bills = crud.bill.filter(db=db, reference=reference,
                             total_from=total_from, total_to=total_to)
    return bills
...
def create_bills(
    ...
    if total != total_subbills:
        raise HTTPException(
            status_code=400, detail="Bill total does not match subbills amount")
    ...
    if len(subbills_exist) > 0:
        ...
        raise HTTPException(
            status_code=400, detail="SubBills with reference already exist: " + ",".join(references))
